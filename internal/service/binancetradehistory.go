...
type (
	IBinanceTraderHistory interface {
		...
		CreateUser(ctx context.Context, address, apiKey, apiSecret string, dai uint64, num, first float64, three, four int64) error
		// SetUser set user
		SetUser(ctx context.Context, address, apiKey, apiSecret string, apiStatus, dai uint64, num, first float64, three, four int64) error
